<script lang="ts">
	...
	// PROTOTYPE (issue #56): the signed-out pages sit outside the app shell. Today they render
	// inside it, nav tabs and all — worth keeping when the redesign lands.
	const SIGNED_OUT = ['/login', '/setup'];
	const bare = $derived(SIGNED_OUT.includes(page.url.pathname));
