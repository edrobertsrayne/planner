...
<script lang="ts">
	...
	const isSelected = (row: AgendaRow) =>
		!!selected &&
		selected.classId === row.classId &&
		selected.date === row.date &&
		selected.period === row.periodFrom;
				...
				<li>
					<button
						...
						class="flex w-full gap-3 px-4 {dense
							...
							? 'bg-muted ring-1 ring-ring ring-inset'
							: ''}"
						onclick={() => onOpen({ classId: row.classId, date: row.date, period: row.periodFrom })}
					>
