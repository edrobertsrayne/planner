<!--
	PROTOTYPE — throwaway. Floating bar. Two axes this time: which screen (the Classes list or the
	Class page) and which variant of it, because #63 asks about both and they have to be compared
	as a pair — a list design only makes sense against the detail page it opens into.
-->
<script lang="ts">
...
>
	<div class="flex items-center rounded-full bg-white/10 p-0.5 dark:bg-black/10">
		{#each [['list', 'Classes'], ['detail', 'Class page']] as [key, label] (key)}
			<button
				type="button"
				onclick={() => nav({ screen: key, variant: 'A' })}
				class="rounded-full px-2.5 py-0.5 text-xs font-medium {screen === key
