<script lang="ts">
	...
	// Slot's own tile, so the teacher can see which one they picked (issue #192). Its only
	// caller is the block-Slot case, so the case is what it takes — the picked Slot — and the
	// action, the fields and the wording are the component's own. The note is the whole point
	// of a Blocked Slot — a hole in the week is otherwise unexplainable months later — so it
	// is required. It stays its own file for the one rule the page must not lose: a note the
	// server rejects stays on screen with what was typed still in it, and the refusal is
	// named, rather than being discarded.
	let {
		pick,
		onOpenChange
	}: {
		pick: { classId: string; classLabel: string; date: string; slotId: string; period: number };
		onOpenChange: (open: boolean) => void;
	} = $props();

	const id = $props.id();
	const label = $derived(`Block ${pick.classLabel}, P${pick.period}`);
...
<!-- Open for exactly as long as the pick it serves: the page renders this only while a Slot
     is picked, so closing unmounts it and a fresh pick starts an empty note. bits-ui closes
     on Escape, but the unmount takes its own trigger with it, so the focus it hands back
     dies on the body — the page returns focus to the day's menu trigger instead. -->
